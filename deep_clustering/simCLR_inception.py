...
transform_standard = transforms.Compose([transforms.ToTensor(), 
                                transforms.ConvertImageDtype(torch.float),transforms.Resize(299), transforms.Normalize(mean=[0.485, 0.456, 0.406] ,std=[0.229, 0.224, 0.225])])
...
train_loader = torch.utils.data.DataLoader(augdataset, batch_size=212, shuffle=True, pin_memory=True,num_workers=8, drop_last=True)
...
class ContrastiveModel(nn.Module):
    ...
    def forward(self, x):
        x = self.backbone(x)
        features = self.contrastive_head(x.squeeze())
...
#model= torch.load("simCLR212_inception.pth")
model.to(dev)
...
out.to_csv("featuresSimCLR212_inception.csv")
...
out.to_csv("latentSimCLR212_inception.csv")

torch.save(model, "simCLR212_inception.pth")
