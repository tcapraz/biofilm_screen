...
transform_standard = transforms.Compose([transforms.ToTensor(), 
                                transforms.ConvertImageDtype(torch.float),transforms.Resize(224), transforms.Normalize(mean=[0.485, 0.456, 0.406] ,std=[0.229, 0.224, 0.225])])
...
train_loader = torch.utils.data.DataLoader(augdataset, batch_size=512, shuffle=True, pin_memory=True,num_workers=8, drop_last=True)
...
class SimCLRLoss(nn.Module):
    ...
    def forward(self, features):
        ...
        assert(n == 2)
        mask = torch.eye(b, dtype=torch.float32).to("cuda:0", non_blocking=True)
        ...
        logits_mask = torch.scatter(torch.ones_like(mask), 1, torch.arange(b).view(-1, 1).to("cuda:0", non_blocking=True), 0)
...
class ContrastiveModel(nn.Module):
    def __init__(self, head='mlp', features_dim=128, hidden_dim=1000):
        ...
        for i in self.backbone.layer1:
            for param in i.parameters():
                param.requires_grad = False

        
        for i in self.backbone.layer2:
            for param in i.parameters():
                param.requires_grad = False
 
        
        for i in self.backbone.layer3:
            for param in i.parameters():
                param.requires_grad = False
...
#model= torch.load("simCLR212_norm.pth")
...
def compute_latent(model, train_loader, N =28476,  batch=128):
    for i, (data, _)  in enumerate(train_loader):
        with torch.no_grad():
            data = data.to(dev, non_blocking=True)
            aux = model.module.get_latent(data)
            aux = aux.to("cpu")
            if i == 0:
                features = np.zeros((N, aux.shape[1])).astype('float32')
...
out.to_csv("featuresSimCLR_freeze.csv")
...
out.to_csv("latentSimCLR_freeze.csv")

torch.save(model, "simCLR_freeze.pth")
