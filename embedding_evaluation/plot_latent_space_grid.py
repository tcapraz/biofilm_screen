...
trainpath ="../data/all_data_bg_png/"
...
biofilm_names = pd.read_csv("../autoencoders/biofilm_image_names.csv", header=None)

transform = transforms.Compose([transforms.ToTensor(), 
                                transforms.ConvertImageDtype(torch.float)])

fulldataset = datasets.ImageFolder(trainpath, transform=transform)
samplenames = np.array([i[0].split("/")[-1].split(".")[0] for i in fulldataset.imgs])
indices = np.where(np.in1d(samplenames, biofilm_names))[0]

fulldataset =torch.utils.data.Subset(fulldataset, indices)

traindataset = datasets.ImageFolder(trainpath, transform=transform)
traindataset = torch.utils.data.Subset(traindataset, indices)
print("Number of samples in training set: ", len(traindataset))
...
vae = torch.load("model_clahe.pth",map_location=torch.device('cpu'))
...
def reconstruct_img(vae, x):
    ...
    loc_img = torch.reshape(loc_img,x.shape)
    return loc_img
...
for i in range(LF):
    ...
    loc_img = torch.reshape(loc_img,(10,3,160,160)).detach().numpy()
...
for i in range(10):
    for j in range(50):
        ...
        ax[i,j].axis('off')
plt.savefig("latent_space_reconstruction.png", dpi=500)
...
pca_img = torch.reshape(pca_img,(20,20,3,160,160)).detach().numpy()
...
for i in range(20):
    for j in range(20):
        ...
        # ax1.axes.yaxis.set_visible(False)
        ax[i,j].axis('off')
plt.savefig("pca_latent_space_reconstruction.png", dpi=500)
