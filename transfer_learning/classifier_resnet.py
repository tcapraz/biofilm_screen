...
data_generator = datagen.flow_from_directory('../data/annotated2_clahe', classes=["discard",  "struct1",  "struct3", "struct4", "struct5", "struct6", "struct7", "struct8", "struct9", "struct10", "struct11", "struct12"],target_size=(224,224), color_mode="rgb", class_mode="input", batch_size=1, shuffle=False)
data_list = []
batch_index = 0

while batch_index <= data_generator.batch_index:
    data = data_generator.next()
    data = preprocess_input(data[0].reshape(224,224,3))
...
for train, test in kf.split(X,y):
    #inp = keras.layers.Input(shape=(224, 160, 3), name='image_input')
    
    vgg_model = ResNet50V2(weights='imagenet', include_top=True)
...
plt.ylabel("TPR")
plt.legend()
plt.savefig("roc2_clahe_resnet.png", dpi=400)
plt.show()
plt.close()
...
plt.xlabel("Recall")
plt.legend()
plt.savefig("precision_recall2_clahe_resnet.png", dpi=400)
plt.show()
plt.close()
...
plot_confmatrix(y_pred_class, y_true_class, title="cm_transfer2_clahe_resnet.png")
