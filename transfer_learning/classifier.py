...
data_generator = datagen.flow_from_directory('../data/annotated3_clahe', classes=["discard",  "struct1",  "struct3", "struct4", "struct6", "struct8", "struct9", "struct10", "struct11"],target_size=(224,224), color_mode="rgb", class_mode="input", batch_size=1, shuffle=False)
data_list = []
batch_index = 0
...
#score_df = pd.DataFrame(np.vstack((y_scores_all,y_true_class)).T, columns=["score", "true"])
#score_df.to_csv("cv_scores2.csv", index =False)
...
plt.xlabel("Recall")
plt.legend()
plt.savefig("precision_recall3.png", dpi=400)
plt.show()
plt.close()
...
plot_confmatrix(y_pred_class, y_true_class, title="cm_transfer3.png")
