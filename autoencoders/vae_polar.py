...
class Decoder(nn.Module):
    def __init__(self, z_dim, in_dim=96,filters=[32, 64, 128]):
        ...
        self.unflatten = nn.Unflatten(1, (filters[2], 43, 3))
...
class VAE(nn.Module):
    ...
    def model(self, x):
        ...
        with pyro.plate("data", x.shape[0]):
            ...
            sigmas = torch.ones(x.shape[0],3*360*40, dtype=x.dtype, device=x.device)*0.1
            pyro.sample(
                "obs",
                dist.Normal(loc_img, sigmas, validate_args=False).to_event(1),
                obs=x.reshape(-1, 3*360*40),
            )
    ...
    def reconstruct_img(self, x):
        ...
        loc_img = torch.reshape(loc_img, (3,360,40))
        return loc_img
...
torch.save(vae, "model_polar.pth")
...
out_tsne.to_csv("tsne_polar.csv")
