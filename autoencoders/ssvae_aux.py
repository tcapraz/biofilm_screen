...
class VAE(nn.Module):
    ...
    def get_latent_rep(self, x):
        alpha = self.encoder_y(x)
        y = torch.tensor(np.argmax(alpha.detach().numpy(), axis=1))
...
#valsize = 100
#train_set, val_set = torch.utils.data.random_split(supervised_data , [len(supervised_data )-valsize, valsize])

labels = np.array(supervised_data.targets)

for i in holdout_paths:
    traindataset.imgs.remove(i)
#traindataset  = torch.utils.data.Subset(traindataset, range(3000))

# valsize = 10000
# train_set, val_set = torch.utils.data.random_split(traindataset, [len(traindataset)-valsize, valsize])

unsupervised_loader = torch.utils.data.DataLoader(traindataset, batch_size=32, shuffle=False)
supervised_loader = torch.utils.data.DataLoader(supervised_data, batch_size=32, shuffle=False)
...
for epoch in range(50):
    ...
    for x, y in supervised_loader:
        y = F.one_hot(y,num_classes=6)
        supervised_loss += svi.step(x.to(dev, non_blocking=True),y.to(dev, non_blocking=True))
        aux_loss += svi_aux.step(x.to(dev, non_blocking=True),y.to(dev, non_blocking=True))
    # report training diagnostics
    normalizer_train = len(unsupervised_loader.dataset)
    normalizer_supervised = len(supervised_loader.dataset)
    ...
    pred_proba = compute_class_proba(vae, supervised_loader, N = len(supervised_data), batch=32)
...
def compute_features(model, train_loader, N =3000,  batch=128):
...
labels = supervised_data.targets
acc = accuracy_score(labels,pred_class)
