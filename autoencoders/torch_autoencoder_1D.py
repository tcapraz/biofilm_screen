...
class ConvAutoencoder(nn.Module):
    def __init__(self, in_dim=96, pools =2, filters1=16, filters2=4, z_dim=50):
        ...
        self.fc1 = nn.Linear(int(fc_dim), 1000)
        self.fc2 = nn.Linear(1000, int(z_dim))

        ## decoder layers ##
        ## a kernel of 2 and a stride of 2 will increase the spatial dims by 2
        self.t_fc2 = nn.Linear(int(z_dim), 1000)
        self.t_fc1 = nn.Linear(1000, 24*24*64)

        self.unflatten = nn.Unflatten(1, (filters2, int(in_dim/(pools)**2),int(in_dim/(pools**2))))
        #self.unflatten = nn.Unflatten(1, (filters2, 23,23))
        self.t_conv1 = nn.ConvTranspose2d(filters2, filters1, pools, stride=pools)
        self.t_conv2 = nn.ConvTranspose2d(filters1, 1, pools, stride=pools)
