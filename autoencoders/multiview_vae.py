...
fulldataset = datasets.ImageFolder(trainpath, transform=transform_dict)
...
train_set = AugmentedDataset(traindataset)
fulldataset =AugmentedDataset(fulldataset)
...
for epoch in range(40):
    ...
    for data in train_loader:
        ...
        epoch_loss += svi.step(x.to(dev, non_blocking=True), x_cropped.to(dev, non_blocking=True))
    ...
    print(
        ...
        % (epoch, total_epoch_loss_train), flush=True
    )
...
torch.save(vae, "model_multiview.pth")
...
paths = [i[0] for i in train_loader.dataset.dataset.imgs]
names = [Path(i).stem for i in paths]

out = pd.DataFrame(features_params, index=names)
out.to_csv("multiview_vae.csv")
