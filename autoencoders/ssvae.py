...
class Encoder(nn.Module):
    def __init__(self, z_dim, y_dim, in_dim=96, filters=[32, 64, 128]):
        ...
        self.fc2_s = nn.Linear(int(fc_dim), int(z_dim))
    ...
    def forward(self, x, y=None):
        ...
        if y is None:
            ...
            # print(y.shape)
            # print(y)
            z_scale = torch.exp(self.fc2_s(x))
...
class VAE(nn.Module):
    ...
    def __init__(self, z_dim=50,y_dim=6, use_cuda=False):
        super().__init__()
        # create the encoder and decoder networks
        #self.encoder_y = Encoder_y(y_dim)
        self.encoder = Encoder(z_dim, y_dim)
...
def compute_class_proba(model, loader, N =3000,  batch=128):
    for i, (data, _)  in enumerate(loader):
        with torch.no_grad():
            ...
            aux = model.encoder.forward(data)
            aux = aux.to("cpu")
            if i == 0:
...
for epoch in range(100):
    ...
    for x, _ in unsupervised_loader:
        ...
        epoch_loss += svi.step(x.to(dev, non_blocking=True), y=None)
