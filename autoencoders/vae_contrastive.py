...
augdataset = datasets.ImageFolder(trainpath, transform=transform_dict)

augdataset = AugmentedDataset(augdataset)

train_loader = torch.utils.data.DataLoader(augdataset, batch_size=521, shuffle=True)


# setup the VAE
#vae = VAE(use_cuda=on_gpu)
vae = torch.load("vae_contrastive.pth")
# setup the optimizer
adam_args = {"lr": 1.0e-5}
optimizer = pyro.optim.Adam(adam_args)
det_opt = torch.optim.Adam(vae.encoder.parameters(), lr= 1.0e-2)
scheduler = pyro.optim.ExponentialLR({'optimizer': optimizer, 'optim_args': adam_args, 'gamma': 0.9 })
...
for epoch in range(50):
    ...
    for x in train_loader:
        standard = x['image'].to(dev, non_blocking=True)
        aug = x['image_augmented'].to(dev, non_blocking=True)
        # do ELBO gradient and accumulate loss
        vae_loss += svi.step(standard)
        z_standard,_ = vae.encoder(standard)
        z_aug,_ = vae.encoder(aug)
        z_standard = F.normalize(z_standard, dim = 1)
        z_aug = F.normalize(z_aug, dim = 1)

        z = torch.cat([z_standard.unsqueeze(1), z_aug.unsqueeze(1)], dim=1)
        det_opt.zero_grad()

        loss = criterion(z)
        torch.nn.utils.clip_grad_norm_(vae.encoder.parameters(), 100)
        loss.backward()
        det_opt.step()
    ...
    normalizer_batch = len(train_loader)
    total_epoch_loss_train = vae_loss / normalizer_train
    total_contrastive_loss =  contrast_loss/normalizer_batch
    train_elbo.append(total_epoch_loss_train)
    print(
        "[epoch %03d]  average training loss: %.2f \n average contrastive loss: %.4f "
        % (epoch, total_epoch_loss_train,total_contrastive_loss  ), flush=True
    )
...
torch.save(vae, "vae_contrastive.pth")

dataset = datasets.ImageFolder(trainpath, transform=transform_standard)

test_loader = torch.utils.data.DataLoader(dataset, batch_size=128, shuffle=False)


features = compute_features(vae,test_loader,  N=len(dataset), batch = 128)

paths = [i[0] for i in test_loader.dataset.samples]
names = [Path(i).stem for i in paths]

out = pd.DataFrame(features, index=names)
out.to_csv("vae_contrastive.csv")
